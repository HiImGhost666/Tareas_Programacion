/*
La pizzería Bella Napoli ofrece pizzas vegetarianas y no vegetarianas a sus clientes. Los ingredientes para cada tipo de pizza aparecen a continuación.

Ingredientes vegetarianos: Pimiento y tofu.
Ingredientes no vegetarianos: Peperoni, Jamón y Salmón.
Escribir un programa que pregunte al usuario si quiere una pizza vegetariana o no, y en función de su respuesta le
muestre un menú con los ingredientes disponibles para que elija.

Solo se puede eligir un ingrediente además de la mozzarella y el tomate que están en todas la pizzas.

Al final se debe mostrar por pantalla si la pizza elegida es vegetariana o no y todos los ingredientes que lleva.
 */
package Tema2.Actividades_ChatGPT;
import java.util.Scanner;
public class Actividad23 {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        System.out.println("¿Quieres una pizza vegetariana o no vegetariana?");
        System.out.println("1. Vegetariana");
        System.out.println("2. No vegetariana");
        int eleccion = sc.nextInt();

        if (eleccion == 1) {
            System.out.println("Vegetariana");
            System.out.println("Ingredientes vegetarianos: Pimiento y tofu");
        } else if (eleccion == 2) {
            System.out.println("No vegetariana");
            System.out.println("Ingredientes no vegetarianos: Peperoni, Jamón y Salmón");
        } else {
            System.out.println("Opción incorrecta");
        }
    }
}
